...
class DataLoader:
    ...
    def __init__(self):
        ...
        self.caixi_list = [
            ['nodeId:ID', 'projectId:int', 'group', 'name', ':LABEL', 'color', 'figure', 'radius:int', 'textSize']]
        self.caipu_list = [
            ['nodeId:ID', 'projectId:int', 'group', 'name', 'properties.口味', 'properties.工艺', 'properties.耗时',
             'properties.做法', ':LABEL', 'color', 'figure', 'radius:int', 'textSize']]
        self.shicai_list = [
            ['nodeId:ID', 'projectId:int', 'group', 'name', 'properties.简介', 'properties.营养价值', 'properties.功效',
    ...
    def addCaixi(self, caixi):
        tmp = [self.nodeId, self.projectId, '菜系', caixi, 'node', '', '', radius, textSize]
        self.nodeId += 1
    ...
    def addCaipu(self, cai):
        tmp = [self.nodeId, self.projectId, '菜谱']
        self.nodeId += 1
    ...
    def addShicai(self, shicai):
        for name in shicai.keys():
            ...
            tmp = [self.nodeId, self.projectId, '食材', name]
    ...
    def load(self, path):
        root = os.getcwd()
        root = root + '/export/'
        caixi_file = open(root + 'caixi1.csv', mode='w', encoding='utf8', newline='')
        caipu_file = open(root + 'caipu1.csv', mode='w', encoding='utf8', newline='')
        shicai_file = open(root + 'shicai1.csv', mode='w', encoding='utf8', newline='')
        zhushicai_file = open(root + 'zhushicai1.csv', mode='w', encoding='utf8', newline='')
        fuliao_file = open(root + 'fuliao1.csv', mode='w', encoding='utf8', newline='')
        belong_file = open(root + 'belong1.csv', mode='w', encoding='utf8', newline='')
